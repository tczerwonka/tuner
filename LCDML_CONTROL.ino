...
// settings
  #define _LCDML_CONTROL_encoder_pin_a           31 // pin encoder b
  #define _LCDML_CONTROL_encoder_pin_b           33 // pin encoder a
  #define _LCDML_CONTROL_encoder_pin_button      35 // pin taster
  #define _LCDML_CONTROL_encoder_quit            41 // stop button on display -- quit
...
// global defines
  ...
  uint8_t  g_LCDML_CONTROL_encoder_q_prev = 0;
...
void LCDML_CONTROL_setup()
{
  ...
  pinMode(_LCDML_CONTROL_encoder_quit     , INPUT_PULLUP);
...
{    
  ...
  unsigned char q = digitalRead(_LCDML_CONTROL_encoder_quit);
  ...
  if (!a && g_LCDML_CONTROL_encoder_a_prev) {
    ...
    g_LCDML_CONTROL_encoder_q_prev = 1;
    ...
    if((millis() - g_LCDML_DISP_press_time) >= _LCDML_DISP_cfg_button_press_time) {
      ...
      if (!t && g_LCDML_CONTROL_encoder_t_prev == 0) {          
          LCDML_BUTTON_enter();          
      } else if (!q && g_LCDML_CONTROL_encoder_q_prev == 0) {
          LCDML_BUTTON_quit();
      }
      else {
        g_LCDML_CONTROL_encoder_t_prev = 0;
        g_LCDML_CONTROL_encoder_q_prev = 0;
      }
    }      
  }
